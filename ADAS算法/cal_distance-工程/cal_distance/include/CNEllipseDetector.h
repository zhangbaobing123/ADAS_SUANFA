...
public:
	...
	void DetectAfterPreProcessing(vector<Ellipsed>& ellipses, Mat1b& E, Mat1f& PHI);

	//Detect the ellipses in the gray image
	void Detect(Mat1b& gray, vector<Ellipsed>& ellipses);

	//Draw the first iTopN ellipses on output
	void DrawDetectedEllipses(Mat& output, vector<Ellipsed>& ellipses, int iTopN = 0, int thickness = 2);
...
private:
	...
	void ClusterEllipses(vector<Ellipsed>& ellipses);
