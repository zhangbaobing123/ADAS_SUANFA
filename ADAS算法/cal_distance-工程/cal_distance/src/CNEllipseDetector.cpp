...
{//根据三段弧计算椭圆参数，并计算得到的椭圆参数的准确定。
	...
	Ellipsed ell(a0, b0, fA, fB, fmod(rho + float(CV_PI)*2.f, float(CV_PI)));
...
void CNEllipseDetector::DetectAfterPreProcessing(vector<Ellipsed>& ellipses, Mat1b& E, Mat1f& PHI)
{
	// Set the image size
	_szImg = E.size();
...
void CNEllipseDetector::Detect(Mat1b& I, vector<Ellipsed>& ellipses)
{
...
void CNEllipseDetector::ClusterEllipses(vector<Ellipsed>& ellipses)
{
	float th_Da = 0.1f;
	float th_Db = 0.1f;
	float th_Dr = 0.1f;
...
void CNEllipseDetector::DrawDetectedEllipses(Mat& output, vector<Ellipsed>& ellipses, int iTopN, int thickness)
{
	int sz_ell = int(ellipses.size());
	int n = (iTopN == 0) ? sz_ell : min(iTopN, sz_ell);
	for (int i = 0; i < n; ++i)
	{
		Ellipsed& e = ellipses[n - i - 1];
...
//void CNEllipseDetector::DrawDetectedEllipses(Mat3b& output, vector<Ellipsed>& ellipses, int iTopN, int thickness)
//{
//	int sz_ell = int(ellipses.size());
//	int n = (iTopN == 0) ? sz_ell : min(iTopN, sz_ell);
//	for (int i = 0; i < n; ++i)
